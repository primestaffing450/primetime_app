...
const api = {
    ...
    approveTimesheet: async (userId, token) => {
        try {
            const response = await axios.put(`${API_URL}/manager/timesheets/${userId}/approve`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
            return { success: true, data: response.data };
        } catch (error) {
            return { success: false, message: error.response?.data?.detail || 'Failed to approve timesheet', status: error?.response?.status };
        }
    }
};
